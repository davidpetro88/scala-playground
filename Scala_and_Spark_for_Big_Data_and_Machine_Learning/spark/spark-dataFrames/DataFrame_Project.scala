...
val dfmax = df2.select($"Year",$"High").groupBy("Year").max()
dfmax.select($"Year",$"max(High)").orderBy($"Year").show()

// What is the average Close for each Calender Month?
val df2Month = df.withColumn("Month",month(df("Date")))
val dfavgs = df2Month.select($"Month",$"Close").groupBy("Month").mean()
